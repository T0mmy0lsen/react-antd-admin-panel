...
export default class OverrideScheduled extends React.Component<any, any> {
    ...
    build() {
        ...
        const cycle: Cycle = main.$cycle(`/override/scheduled/${faculty}`);
        const get: Get = main.$get(cycle, `/api/courses/override/scheduled-date`);

        const explain: string = 'Listen indeholder ændringer af oprettelsesdatoer for planlagte kurser.';

        const section = new Section();
        const scheduled = new List()
            .get(() => get)
            ...
            .actions(new Action()
                ...
                .formula(new Formula(new Post()
                    .target((args) => ({
                        method: 'DELETE',
                        target: `/api/courses/override/scheduled-date/${args.record.courseId}`
                    }))
        ...
        section.add(new Title().label('Ændringer af Oprettelsesdato').level(1));
