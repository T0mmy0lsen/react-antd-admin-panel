...
export default class App extends React.Component<any, any> {
    ...
    getHeader() {
        ...
        return (
            <Row justify={'start'} style={{ height: 62 }}>
                <Row style={{ width: '100%' }}>
                    <Row justify={'start'} style={{ width: '50%', background: '#f0f0f0' }}>
                        <Header
                            main={this.main}
                            logo={this.main.$config.config?.pathToLogo}
                        />
                    </Row>
                    <Row justify={'end'} style={{ width: '50%', background: '#f0f0f0' }}>
                        {
                            this.state.initiated &&
                            this.main.$config.config.profile &&
                            <Layout.Header className="site-layout" style={{ padding: '1px 0 0 0', marginRight: 36 }}>
                                <Button
                                    icon={<FontAwesomeIcon icon={faGears} />}
                                    style={{ padding: 0, color: '#868686', background: '#f0f0f0', borderColor: '#f0f0f0', boxShadow: 'none', width: 36, height: 36, borderRadius: '50%', textAlign: 'center', fontSize: 16, marginRight: 12 }}
                                >
                                </Button>
                                <Button
                                    style={{ padding: 0, color: '#000', width: 36, height: 36, borderRadius: '50%', textAlign: 'center', fontSize: 16 }}
                                >
                                    <div>{getInitials(this.main.User._object.name)}</div>
                                </Button>
                            </Layout.Header>
                        }
                    </Row>
            ...
            <BrowserRouter>
                ...
                && (
                    <Modal
                        ...
                        mask={this.state.modal.mask}
                        maskClosable={this.state.modal.maskClosable}
                        ...
                        cancelText={this.state.modal.cancelText ?? 'Afbryd'}
                        okText={this.state.modal.okText ?? 'Ok'}
                    >
                ...
                { this.state.initiated && (
                    <Layout style={{ backgroundColor: 'transparent' }}>
                        ...
                        { this.main.$config.drawer
                            ? (
                                <Layout.Sider
                                    ...
                                    width={80}
                                    style={{ minHeight: 'calc(100vh - 0px)', height: '100%', left: 0, position: 'fixed', zIndex: 10, marginTop: 0 }}
                                >
                        ...
                        <Layout className="site-layout" style={{
                            ...
                            marginLeft: this.state.collapsed ? 0 : 80,
                        }}>
                            {this.getHeader()}
                                ...
                                <Row justify={'start'}>
                                    <Col span={24} style={{ background: '#f0f0f0' }}>
                                        <Layout.Content style={{
                                            ...
                                            borderLeft: '1px solid rgba(5,5,5,.10)' ,
                                            borderTop: '1px solid rgba(5,5,5,.10)' ,
                        ...
                        >
                            <Row>
                                ...
                                <Layout style={{ backgroundColor: '#d3d3d326', margin: 0, width: 480, minHeight: '100vh', height: '100%' }}>
                                    <Layout.Header className="site-layout" style={{ padding: 0, height: 62 }}>
                                        <Row justify={'end'} style={{ marginRight: 24, marginTop: 20 }}>
                                            <Tooltip title="Close">
                                                <Button
                                                    shape="circle"
                                                    size={"small"}
                                                    icon={<CloseOutlined style={{ opacity: 0.6 }}/>}
