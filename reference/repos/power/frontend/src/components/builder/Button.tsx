...
const Button = (props: any) => {
    ...
    const Button = () => {
        ...
        const Icon = (model: ButtonModel, marginRight: number = 8) => {
            if (model._action?._fontawesome) return (<span style={{ width: 20 }}><FontAwesomeIcon style={{ opacity: .6, marginRight: marginRight, fontSize: model._style.fontSize ?? '16px' }} icon={model._action._fontawesome} /></span>)
            if (model._fontawesome) return (<span style={{ width: 20 }}><FontAwesomeIcon style={{ opacity: .6, marginRight: marginRight, fontSize: model._style.fontSize ?? '16px' }} icon={model._fontawesome} /></span>)
        ...
        return hide ? null : <ButtonAnt
                ...
                onClick={() => onClick()}
                icon={Icon(model)}
                // iconPosition={model._iconPosition ?? 'left'}
