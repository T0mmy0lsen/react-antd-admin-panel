...
export default class Get extends Axios {
    ...
    finalize(response: any, args?: any, callback?: any, filename?: any)
    {
        ...
        // Check if response is a blob (for file download)
    ...
    get(args: any = undefined, callback: any = undefined, filename = 'file.xlsx') {
        ...
        if (target.responseType) responseType = target.responseType;
        ...
        axios[method](target, settings).then((response: any) => {
            ...
            this.finalize(response, args, callback, filename)
        })
        .catch((e: any) => {
            if (!this._mock) {
