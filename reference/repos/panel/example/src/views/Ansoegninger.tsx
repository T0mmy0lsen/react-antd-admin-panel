...
export default class Jobopslag extends React.Component<any, any> {
    ...
    build() {
        ...
        const cycle: Cycle = main.$cycle(`/ansoegning`);
        const get: Get = main.$get(cycle, `/ansoegning`);
        ...
        const things = new List()
            ...
            .emptyColumn(true)
            .headerCreate(false)
            .headerPrepend(new ListHeader().key('name').title('name').searchable())
            .headerPrepend(new ListHeader().key('phaseName').title('phaseName').filterable())
            .headerPrepend(new ListHeader().key('stateName').title('stateName').filterable())
            .actions(new Action()
                .icon(ArrowRightOutlined)
                .callback((v) => {
                    main.$route(`/jobopslag/${v.reqId}`);
                })
            );

        section.style({ padding: '24px 36px' });
        section.add(new Title().label('Ansøgninger').level(1));
